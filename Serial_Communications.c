...
void heat_beat_packet_back(void)
{
    ...
    else if (Current_temperature < 70)
    {
        T_sms_buf[5] =0x07;             //温度小于60度
    }
    else if( (Offer_Step >1) || F_channel_clear_ing || F_cup_down_ing )
    {
        T_sms_buf[5] =0x08;             //忙
    }
...
//      wmq_status =R_sms_buf[3];
    ...
    else if ((R_sms_buf[1]==0x05) && (R_sms_buf[2]==0x02))                //扫描支付命令
    {
        ...
        if ((sms_4==0) && (sms_3>0) && (sms_3<9) && (!F_sys_err) && (Offer_Step <2) && (P_Set==F_P_Set_init_bak))
        {
    ...
    else if ((R_sms_buf[1]==0x10) && ( R_sms_buf[2] ==0x02 ))  //维护命令
    {
        T_sms_buf[1] =0x90;                 //应答维护命令命令
        T_sms_buf[2] =0x02;                 //应答维护命令命令 数据长度
        uchar sms_3,sms_4;
        sms_3= R_sms_buf[3];
        sms_4= R_sms_buf[4];

        if ((sms_host_maintain==0) && (P_Set==F_P_Set_init_bak) && (Offer_Step <2))
        {
            sms_host_maintain =sms_3;
            T_sms_buf[3] =sms_3;
            T_sms_buf[4] =sms_4;
        }
        else{
    ...
    else if(( R_sms_buf[1]==0x90 ) && ( R_sms_buf[2] ==0x02 ))           //维护命令确认
    {
